export const CONSTANTS = {
  LESSONS: [
    ...
    },
    {
      ...
      answer: 'В ангуляр есть два вида созддания форм - <b>Template-driven forms</b> и <b>Reactive forms</b>. ' +
        '<b>Template-driven forms</b> это более старый подход к созданию форм в котором ключевую роль играет шаблон ' +
        'компонента и все описание производится в нем. <b>Reactive forms</b> - новый подход для работы с формами в ' +
        'реактивном стиле. Описание формы происходит в компоненте в виде дерева объектов, после чего это дерево ' +
        'связывается с шаблоном. Все манипуляции (проверка валидности, подписка на изменение значения и прочее) ' +
        'производятся в компоненте, что делает работу более гибкой, удобной и предсказуемой. Именно поэтму подход ' +
        '<b>Reactive forms</b> лучший выбор для крупных проэктов.'
    },
    {
      lesson: 5,
      question: 'Describe component lifecycle hooks: OnInit, OnChanges, AfterViewInit',
      answer: [
        '<b>ngOnChanges</b>: вызывается до метода ngOnInit() при начальной установке свойств, которые связаны механизмом ' +
        'привязки, а также при любой их переустановке или изменении их значений. Данный метод в качестве параметра ' +
        'принимает объект класса SimpleChanges, который содержит предыдущие и текущие значения свойства.',
        '<b>ngOnInit</b>: вызывается один раз после установки свойств компонента, которые участвуют в привязке. ' +
        'Выполняет инициализацию компонента. <br>' +
        'Метод ngOnInit() применяется для какой-то комплексной инициализации компонента. Здесь можно выполнять загрузку ' +
        'данных с сервера или из других источников данных. <br>' +
        'ngOnInit() не аналогичен конструктору. Конструктор также может выполнять некоторую инициализацию объекта, в ' +
        'то же время что-то сложное в конструкторе делать не рекомендуется. Конструктор должен быть по возможности ' +
        'простым и выполнять самую базовую инициализацию. Что-то более сложное, например, загрузку данных с сервера, ' +
        'которая может занять продолжительное время, лучше делать в методе ngOnInit.',
        '<b>ngDoCheck</b>: вызывается при каждой проверке изменений свойств компонента сразу после методов ngOnChanges и ' +
        'ngOnInit',
        '<b>ngAfterContentInit</b>: вызывается один раз после метода ngDoCheck() после вставки содержимого в ' +
        'представление компонента кода html',
        '<b>ngAfterContentChecked</b>: вызывается фреймворком Angular при проверке изменений содержимого, которое ' +
        'добавляется в представление компонента. Вызывается после метода ngAfterContentInit() и после каждого ' +
        'последующего вызова метода ngDoCheck().ngAfterContentChecked: вызывается фреймворком Angular при проверке ' +
        'изменений содержимого, которое добавляется в представление компонента. Вызывается после метода ' +
        'ngAfterContentInit() и после каждого последующего вызова метода ngDoCheck().',
        '<b>ngAfterViewInit</b>: вызывается фреймворком Angular после инициализации представления компонента, а также ' +
        'представлений дочерних компонентов. Вызывается только один раз сразу после первого вызова метода ' +
        'ngAfterContentChecked()',
        '<b>ngAfterViewChecked</b>: вызывается фреймворком Angular после проверки на изменения в представлении ' +
        'компонента, а также проверки представлений дочерних компонентов. Вызывается после первого вызова метода ' +
        'ngAfterViewInit() и после каждого последующего вызова ngAfterContentChecked()',
        '<b>ngOnDestroy</b>: вызывается перед тем, как фреймворк Angular удалит компонент.'
      ]
    },
    {
      lesson: 5,
      question: 'Describe built-in validation in Angular',
      answer: 'built-in validation - это встроенные валидаторы для валидации форм в ангуляр. Существвуют такие ' +
        'стандартные валидаторы как min, max, minLength, maxLength, required, requiredTrue, email, pattern, ' +
        'nullValidator, compose, composeAsync.'
    },
    {
      lesson: 5,
      question: 'What is formControl?',
      answer: 'При исспользовании Reactive Forms подхода к соозданию форм создается набор объектов FormGroup и ' +
        'FormControl. Сама форма и ее подсекции представляют класс FormGroup, а отдельные элементы ввода - класс ' +
        'FormControl. <br><br><b><code>' +
        'myForm : FormGroup = new FormGroup({<br>\n' +
        '&nbsp;&nbsp;"userName": new FormControl(),<br>\n' +
        '&nbsp;&nbsp;"userEmail": new FormControl(),<br>\n' +
        '&nbsp;&nbsp;"userPhone": new FormControl()<br>\n' +
        '});</code></b>'
    }
  ]
}
